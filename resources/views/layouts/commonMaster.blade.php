...
<head>
    ...
    <!-- Favicon -->
    @if (!empty($dashboardFaviconUrl))
        <link rel="icon" type="image/png" sizes="32x32" href="{{ $dashboardFaviconUrl }}" />
        <link rel="shortcut icon" href="{{ $dashboardFaviconUrl }}" />
    @else
        <link rel="icon" type="image/x-icon" href="{{ asset('assets/img/favicon/favicon.ico') }}" />
    @endif
    @if (!empty($dashboardAppleTouchIconUrl))
        <link rel="apple-touch-icon" sizes="180x180" href="{{ $dashboardAppleTouchIconUrl }}" />
    @endif
